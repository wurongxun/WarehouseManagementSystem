...
$(function () {
        ...
        columns: [[
            {
                ...
                align: 'center',
                formatter: function (value, row, index) {
                    var str = '<a  name="view" class="easyui-linkbutton"  onclick="Approval_OK('
                        + index + ')">审核通过</a>'
                        + ' <a name="view2" class="easyui-linkbutton"  onclick="Approval_NO(' + index + ')">审核不通过</a>';
                    return str;
                },
            }
        ]],
        url: "detailedListApprovalController.html?act=list&detailedListType=1&status=2",
        method: "GET",
        onClickRow: function () {
            $('#detailedListApproval_Warehousing_table').datagrid('clearSelections');//禁止选中行
        },
    ...
    $("#detailedListApproval_Warehousing_table").datagrid(datagridDataOptions);
    ...
    // });
    // 初始化分页器
    var pager = $("#detailedListApproval_Warehousing_table").datagrid("getPager");
...
function loadDatagrid_ApprovalWarehousing() {
    $("#detailedListApproval_Warehousing_table").datagrid("load", {
        detailedListName: $("input[name$='ApprovalWarehousing_detailedListName']").val(),
        auditByName: $("input[name$='ApprovalWarehousing_auditByName']").val(),
        receivingDelivererPersonName: $("input[name$='ApprovalWarehousing_receivingDelivererPersonName']").val(),
        receivingDelivererPersonPhone: $("input[name$='ApprovalWarehousing_receivingDelivererPersonPhone']").val(),
        detailedListCode: $("input[name$='ApprovalWarehousing_detailedListCode']").val(),
    });
}

//查询

function findAction_ApprovalWarehousing() {
    loadDatagrid_ApprovalWarehousing();
}

function enter(event) {
    if (event.which == "13") {
        findAction_ApprovalWarehousing();
    }
}
...
function doClearApprovalWarehousing() {
    $('#DetailedListApproval_Warehousing_list_form_id').form('clear');
    loadDatagrid_ApprovalWarehousing();
}

function Approval_OK(index) {
    var rows = $("#detailedListApproval_Warehousing_table").datagrid('getRows');
        ...
        $.messager.confirm('提示', '确认审核通过?', function (r) {
            if (r) {
                $.ajax({
                    ...
                    url: "detailedListApprovalController.html?act=detailedListApproval",
